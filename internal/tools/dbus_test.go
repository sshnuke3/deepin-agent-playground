...
func TestParseGVariantString_Empty(t *testing.T) {
	// 在 playground 的 gvariant.go 中,(,) 被当成“空”处理，返回空串。
	// 这是与 v4 teams 故意不同的小调整：上游在调应是 string 的 D-Bus 方法，
	// 如果拿到空返回，正确语义是“该值是空”，而不是 "逗号元组"原样。
	got := parseGVariantString("(,)")
	if got != "" {
		t.Errorf("empty tuple should return empty string, got %q", got)
	}
}
